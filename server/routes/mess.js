...
router.post('/menu', auth, async (req, res) => {
  try {
    ...
    const menu = await Menu.findOneAndUpdate(
      { type: 'mess', date: new Date(date) },
      { items, type: 'mess', date: new Date(date) },
      { upsert: true, new: true }
    );

    const agentResponse = await menuAgent('menu_updated', { type: 'mess', date });
...
router.get('/proposals', auth, async (req, res) => {
  try {
    ...
    const proposals = await MenuProposal.find({ type: 'mess' })
